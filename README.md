...
2. バックエンド起動：
   ...
   ```
   起動後、`http://127.0.0.1:8000/docs` にアクセスして Swagger UI が表示されれば OK。
...
5. コマンドラインから単体実行（デバッグ用）：
   - `calc_gpa.py` の `main()` 内の CSV パスを対象ファイルに書き換えて実行：
     ```bash
     python calc_gpa.py
     ```
...
- **GPA 対象外の区分名**: `EXCLUDED_CATEGORY = "C0"`
